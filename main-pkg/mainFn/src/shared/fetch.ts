...
export class Fetch {
  ...
  private async fetchOnePage<T>(
    ...
    const {
      data: graphQlData,
      errors
    } = await this.graphQl<OnePageGraphQlResponse<T>>({query, variables})
    ...
    return flatMap(
      ...
      await Promise.all(
        [...new Array(numberOfPage)]
          .map(
            (item, index) => this.fetchOnePage<T>(query, responseKey, {
              ...variables,
  ...
  private async autoPaginationGraphqlRequestSkedulo<T>(
    ...
    const remainingPage = Math.max(
      Math.ceil((totalCount - defaultOffSet) / this.maxGraphQlPageLimit) - 1,
      0
    )
    const subsequenceData = flatMap(
      ({ data }) => data,
      await Promise.all(
        [...new Array(remainingPage)].map((item, index) => this.fetchOnePage<T>(
          query, responseKey, {
            ...initVariable,
